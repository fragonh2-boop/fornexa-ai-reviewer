...
Conversational Slack mentions use the same provider adapters and a separate capability set limited to public web and weather lookups. The local Antigravity sidecar is never registered as a conversational model tool; it is reachable only through the explicit signed-approval protocol below. Each provider needs its own Slack app identity because Slack mentions resolve to installed users/apps and a router cannot impersonate several identities. The first exact `@bot` mention opens a thread; subsequent human replies continue there. Exact Slack message timestamps provide terminal correlation and restart-safe dedupe. See [Provider mentions in Slack](slack-provider-mentions.md).

Transient provider failures (connection, 408/409/429 and 5xx) receive at most two shared transport retries. An explicit `usa query_local_antigravity para …` Slack request bypasses provider interpretation during a quota incident, but it only creates a two-hour Slack approval button. Execution requires a Slack-signed interaction from `SIDECAR_APPROVER_SLACK_USER_IDS`; a message carrying the same user ID cannot approve itself. Approval tokens use `APPROVAL_HMAC_SECRET`, separate from Slack's signing secret, and bind the exact thread, request and task hash. The sidecar accepts only fixed git status/diff/log/branch, tests/build and bounded positive-list file reads. It rejects symlinks, oversized files, credential-looking content, `.env`, Git internals, credential directories/files, private keys and traversal.

Operational checkpoint (2026-09-24): Render service `srv-daobas6gekts73bjr5s0` reports `autoDeploy=no` and `autoDeployTrigger=off`. Changing that setting did not trigger a deployment; the live commit remained `33f3da6d6781c6fa7ef6de4a5b2bb7b1591bf5b3`.
...
**Implementation activation MUST remains open: shared Slack identity.** An allowlisted root-message user ID still does not prove human authorship for `MODE: IMPLEMENT`; keep implementation writes disabled until agents use separate bot identities or that path gains an interactive approval. Controlled Render/Vercel deployment now uses a separate Slack-signed button interaction bound to exact mode, SHA, thread and expiry, so a shared user token cannot forge deployment approval. A textual "sent using Claude" footer remains non-authentication.
...
Follow-up validation: 88 local tests, TypeScript build and diff-check pass. The current HEAD requires a scoped, independent exact-HEAD review and is not automatically approved. No merge or deployment.
